...
- `cli.py`: argparse command definitions and batch orchestration for all six subcommands
  (`doctor`, `prepare`, `status`, `finalize`, `embed`, `tag`).
...
- `pipeline.py`: `VideoTaggingPipeline` — the `tag` command's analysis flow (YOLOv11 objects +
  YuNet/SFace faces -> `EvidenceAccumulator` gating -> embedded `Auto Tags/Video` metadata).
...
- `digikam_db.py`: read-only digiKam catalog access. `DigiKamCatalog` reads confirmed face
  ...
  recognizer.
- `metadata.py`: `ExifToolMetadataWriter` — merges sidecar and requested XMP into writable media,
  verifies the media, preserves timestamps, then sends the sidecar to the Recycle Bin.
- `jobs.py`: proxy job manifests, completion ledger, manifest-owned cleanup, and rerun skipping.
- `config.py`: environment-driven defaults, `ToolPaths`, and `DatabaseConfig`.
...
`doctor` validates FFmpeg/CUDA, OpenCV OpenCL, ExifTool, models, and database access.
...
Never write directly to digiKam’s database. Treat it as read-only. Embed XMP only through
`ExifToolMetadataWriter`; recycle adjacent sidecars only after read-back verification, and retain
them on every failure. Do not commit personal paths, credentials, databases, models, generated
frames, or media. Keep workstation settings in ignored `config.local.ps1`. Proxy cleanup must
target only files recorded by a validated job manifest.
